...
import { validatePayload } from './whatsapp-homolog-receiver.mjs';

// A credencial 360dialog trafega somente no stdin do SSH, nunca em argv/arquivo.
let directory;
try {
  ...
  const payload = validatePayload({ version: 1,
    ...
    DIALOG_360_WEBHOOK_SECRET: process.env.DIALOG_360_WEBHOOK_SECRET });
  ...
  directory = fs.mkdtempSync(path.join(os.tmpdir(), 'whatsapp-homolog-'));
  ...
  const child = spawn('/usr/bin/ssh', [
    ...
    `whatsapp-homolog-deploy@${host}`, 'apply-whatsapp-homolog-v1',
  ...
  fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,
    ...
    + 'Não valida API key na 360dialog, HTTPS público, persistência ou conversa real. Produção não foi alterada.\n');
} catch {
