...
export const veiculos = mysqlTable(
  ...
  t => [
    ...
    foreignKey({
      name: "veiculos_pessoa_empresa_fk",
...
export const notasFiscais = mysqlTable(
  "notas_fiscais",
  {
    id: bigint("id", { mode: "number", unsigned: true })
      .autoincrement()
...
export const despesas = mysqlTable(
  ...
  t => [
    ...
    index("despesas_empresa_created_at_idx").on(t.empresaId, t.createdAt),
  ]
);
...
export const logAuditoria = mysqlTable(
  "log_auditoria",
  {
    id: bigint("id", { mode: "number", unsigned: true })
      .autoincrement()
  ...
  t => [
    index("log_auditoria_metricas_idx").on(
      t.empresaId,
      t.entidade,
      t.acao,
      t.entidadeId
...
export const colaboradores = mysqlTable(
  "colaboradores",
  {
    ...
    superiorDiretoId: bigint("superior_direto_id", {
      mode: "number",
      unsigned: true,
    }),
  ...
  t => [
    foreignKey({
      name: "colaboradores_superior_tenant_fk",
      columns: [t.empresaId, t.superiorDiretoId],
      foreignColumns: [t.empresaId, t.id],
    }),
...
export const checkinsCampo = mysqlTable(
  ...
  t => [
    index("checkins_campo_colab_tempo_idx").on(
      t.empresaId,
      t.colaboradorId,
      t.registradoEm
    ),
...
export const whatsappInbox = mysqlTable(
  ...
  t => [
    uniqueIndex("whatsapp_inbox_provider_chave_unique").on(
      t.provider,
      t.chaveIdempotencia
    ),
    index("whatsapp_inbox_processamento_idx").on(
      t.status,
      t.proximaTentativaAt
    ),
...
export const whatsappOutbox = mysqlTable(
  ...
  t => [
    uniqueIndex("whatsapp_outbox_provider_chave_unique").on(
      t.provider,
      t.chaveIdempotencia
    ),
    index("whatsapp_outbox_processamento_idx").on(
      t.status,
      t.proximaTentativaAt
    ),
    index("whatsapp_outbox_provider_mensagem_idx").on(
      t.provider,
      t.providerMensagemId
    ),
