    ...
    <div className="flex flex-col gap-3 rounded-xl border border-line bg-surface p-5 shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1.5">
          <span className="flex items-center gap-1.5 text-[12px] font-medium uppercase tracking-[0.04em] text-text-500">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  aria-label={`Sobre ${caption}`}
                  className="text-text-500/70 transition-colors hover:text-text-900"
                >
          ...
          <span
            className={cn(
              "flex items-center gap-2 font-mono text-[30px] font-semibold tabular tracking-[-0.01em]",
              KPI_CORES[cor]
            )}
          >
            <MoneyValue
              ...
              color="neutral"
              animate
              className={cn("!text-[30px]", KPI_CORES[cor])}
            />
            {icone}
          </span>
        </div>
...
function EvolucaoTooltip({
  ...
  return (
    <div className="rounded-xl border border-line bg-surface px-3.5 py-3 shadow-card">
      <p className="mb-2 font-mono text-[11px] uppercase tracking-[0.04em] text-text-500">
        {label}
      </p>
      <div className="flex flex-col gap-1.5">
        {categorias.map(cat => {
          ...
          return (
            <span key={cat} className="flex items-center gap-2 text-[12px]">
              <span
                ...
                style={{ backgroundColor: CATEGORIA_COR[cat] }}
              />
              <span className="flex-1 text-text-500">
                {CATEGORIA_LABEL[cat]}
              </span>
...
export default function Dashboard() {
  ...
  const [serie, setSerie] = useState<"identificado" | "capturavel">(
    "identificado"
  );
  ...
  const politicaAtiva = trpc.politica.ativa.useQuery(
    { empresaId: empresaId ?? 0 },
    { enabled: empresaId !== undefined, retry: false }
  );
  ...
  const faixa = useMemo(
    () => faixaDoPeriodo(periodo, customRange),
    [periodo, customRange]
  );
  ...
  const filtradas = useMemo(
    () =>
      despesas.filter(d => {
        const data = d.dataFatoGerador
          ? parseDataLocal(d.dataFatoGerador)
  ...
  const bucketInfo = useMemo(() => {
    ...
    for (const d of filtradas) {
      const data = d.dataFatoGerador
        ? parseDataLocal(d.dataFatoGerador)
        : new Date(d.createdAt);
      const chave = granularidadeDia
        ? toISODate(data)
        : toISODate(data).slice(0, 7);
      if (!mapa.has(chave)) {
        mapa.set(chave, {
  ...
  const categoriasPresentes = useMemo(() => {
    ...
    return (Object.keys(CATEGORIA_LABEL) as CategoriaDespesa[]).filter(c =>
      set.has(c)
    );
  }, [filtradas]);
          ...
          ponto[cat] = filtradas
            .filter(d => {
              const data = d.dataFatoGerador
                ? parseDataLocal(d.dataFatoGerador)
                : new Date(d.createdAt);
              const chaveD = granularidadeDia
                ? toISODate(data)
                : toISODate(data).slice(0, 7);
              return (
                chaveD === chave &&
                d.categoria === cat &&
                (serie === "identificado" || d.confianca === "alta")
              );
            })
  ...
  const sparks = useMemo(() => {
    const base = new Map<
      string,
      { identificado: number; capturavel: number; revisao: number }
    >();
    for (let i = 11; i >= 0; i--) {
      const data = subMonths(new Date(), i);
      base.set(toISODate(data).slice(0, 7), {
        identificado: 0,
        capturavel: 0,
        revisao: 0,
      });
  ...
  const countAlta = useMemo(
    () => despesas.filter(d => d.confianca === "alta").length,
    [despesas]
  );
        ...
        <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-line bg-surface px-8 py-16 text-center">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="max-w-sm text-sm text-text-500">
              Cadastre uma empresa para começar a identificar créditos
              tributários sobre as despesas.
            </p>
          </div>
          <Link
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col gap-1">
          <h2 className="font-display text-xl font-semibold tracking-[-0.02em] text-text-900">
            Indicadores da operação
          </h2>
          <p className="font-mono text-[12px] tracking-[0.02em] text-text-500">
            {faixa.rotulo}
          </p>
        </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  periodo === p.key
                    ...
                    : "text-text-500 hover:bg-paper hover:text-text-900"
                )}
              >
                {p.rotulo}
              </button>
        ...
        {[
          {
            ...
            tooltip:
              "Soma do potencial de crédito + dedutibilidade detectado, antes de qualquer validação.",
          ...
          },
          {
            caption: "Capturável",
            tooltip:
              "Créditos de alta confiança, apurados e prontos para recuperar.",
          ...
          },
          {
            caption: "Em revisão",
            tooltip:
              "Classificações de média confiança aguardando validação humana antes de efetivar créditos.",
            ...
            rodape: (
              <span className="flex items-center justify-between gap-2">
                <span>
                  ...
                  {(resumo.data?.pendenciasRevisao ?? 0) === 1
                    ? "despesa"
                    : "despesas"}
                </span>
                <Link
      ...
      {semDados ? (
        <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-line bg-surface px-8 py-16 text-center">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="max-w-sm text-sm text-text-500">
              Envie a primeira nota fiscal para o motor classificar créditos e
              dedutibilidade.
            </p>
          </div>
          <Link
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex flex-wrap items-center gap-2.5">
                    {categoriasPresentes.map(cat => (
                      <span
                        key={cat}
                        className="flex items-center gap-1.5 text-[12px] text-text-500"
                      >
                        <span
                          className="h-2 w-2 rounded-full"
                          style={{ backgroundColor: CATEGORIA_COR[cat] }}
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <RechartsTooltip
                        content={
                          <EvolucaoTooltip categorias={categoriasPresentes} />
                        }
                ...
                <>
                  <div className="relative mx-auto mt-2 h-48 w-48">
                      ...
                      <PieChart>
                        ...
                        <RechartsTooltip
                          formatter={(value, name) => [
                            ...
                            CONFIANCA_LABEL[name as NivelConfianca] ??
                              String(name),
                          ]}
                        />
                      </PieChart>
                    ...
                    <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                      <span className="font-mono text-[28px] font-semibold tabular text-text-900">
                        {donutTotal}
                      </span>
                      ...
                      <span
                        className={cn(
                          ...
                          p.destaque === "red"
                            ...
                            : "bg-conf-media-bg text-conf-media-text"
                        )}
                      >
                        {p.icone}
                      </span>
                ...
                <table className="w-full border-collapse text-left">
                  <thead>
                    <tr className="border-y border-line">
                      <th className="h-10 px-5 text-[12px] font-medium uppercase tracking-[0.04em] text-text-500">
                        Data
                      </th>
                      <th className="h-10 px-4 text-[12px] font-medium uppercase tracking-[0.04em] text-text-500">
                        Categoria
                      </th>
                      <th className="h-10 px-4 text-[12px] font-medium uppercase tracking-[0.04em] text-text-500">
                        Fornecedor
                      </th>
                      <th className="h-10 px-4 text-right text-[12px] font-medium uppercase tracking-[0.04em] text-text-500">
                        Valor
                      </th>
                      <th className="h-10 px-4 text-[12px] font-medium uppercase tracking-[0.04em] text-text-500">
                        Confiança
                  ...
                  <tbody>
                    {recentes.map((d, i) => {
                      const Icone = d.categoria
                        ? CATEGORIA_ICON[d.categoria]
                        : Receipt;
                      return (
                        <motion.tr
                          key={d.id}
                          ...
                          onClick={() =>
                            navigate(`/app/despesas?despesa=${d.id}`)
                          }
